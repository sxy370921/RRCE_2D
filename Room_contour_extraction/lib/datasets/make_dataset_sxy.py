...
from .dataset_catalog_sxy import DatasetCatalog_sxy
import torch
...
def _dataset_factory2(data_source, task):
    module = '.'.join(['lib.datasets', data_source, task])
    path = os.path.join('lib/datasets', 'sxy', 'sbd_voc_snake_sxy'+'.py')
    dataset = imp.load_source(module, path).Dataset
    return dataset
...
def make_dataset(cfg, dataset_name, transforms, is_train=True):
    args = DatasetCatalog_sxy.get(dataset_name)
    data_source = args['id']
    dataset = _dataset_factory2(data_source, cfg.task)
    del args['id']
    # args['cfg'] = cfg
    # args['transforms'] = transforms
    # args['is_train'] = is_train
...
def make_data_loader_sxy(cfg, is_train=True, is_distributed=False, max_iter=-1):
    if is_train:
