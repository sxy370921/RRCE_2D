...
def transform_coord(y_coordMat, x_coordMat, rotationCenter, transformVect):
    ...
    #transformVect中位移和旋转向量是针对机器人局部坐标系基于机器人朝向进行的位移和旋转（局部坐标系沿机器人方向是x，垂直于机器人方向是y）。
    # 上面这一步是把transformVect所描述的针对机器人局部坐标系的位移变换到地图坐标系上，计算出为了达到基于局部坐标系transformVect描述的同样的位移效果基于地图坐标系x，y应该位移的距离
    ...
    # print("transform_coord: ",transformVect,transformVect.shape)
    #以上是旋转过程，没有认真解读，但大体能看出来，就是将原来的坐标x_coordMat，先以机器人为中心进行旋转再加上位移量
